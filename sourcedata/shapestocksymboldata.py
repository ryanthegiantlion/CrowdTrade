...
def shape(item):
	return {
	    ...
	    "low": "{0:.2f}".format(float(item['DaysLow'])),
	    "ave": "{0:.2f}".format((float(item['DaysLow']) + float(item['DaysHigh']))/2),
	    "high": "{0:.2f}".format(float(item['DaysHigh'])),
	    "hasIncreased": item['ChangeinPercent'][0] == "+",
	    "current": "{0:.2f}".format(float(item['Ask']) if item['Ask'] else (float(item['DaysLow']) + float(item['DaysHigh']))/2),
